...
class Point2(Vector2, Geometry):
    def __repr__(self):
        return '(%.12f, %.12f)' % (self.y, self.x)

    def __lt__(self, other):
        if isinstance(other, Vector2):
            return self.x < other.x
        elif isinstance(other, LineSegment2):
            return self.x < (other.p.x + other.v.x) or self.x < other.p.x
        else:
            return False
...
class LineSegment2(Line2):
    ...
    # this function was added TODO: please check!
    def __lt__(self,other):
        if isinstance(other,Ray2):
            return self.p.x  < other.p.x or self.p.x + self.v.x < other.p.x

        elif isinstance(other,LineSegment2):
            return (self.p.x + (self.v.x)/2) < (self.p.x + (self.v.x)/2)
        return False
